...
synchronized是 Java 中的关键字，用于保证多线程环境下的数据安全。synchronized的实现原理是基于 Java 对象头中的锁标志位和 monitor 对象实现的。

偏向锁：当一个线程获取 synchronized 锁时，会尝试使用 CAS 操作将对象头中的锁标志位修改为指向自己的线程ID，如果成功，则获取锁成功，此时会把偏向锁标志位设置 1。

下次该线程再次获取锁时，会直接判断对象头中的线程ID是否为自己，如果是，则直接获取锁，不需要再次竞争。如果不是，则需要撤销偏向锁，转换为轻量锁或重量锁。

轻量级锁：是一种针对多线程竞争情况下的优化，它的作用是减少多线程竞争情况下的锁开销。当一个线程获取锁时，会尝试使用CAS操作将对象头中的锁标志位修改为指向自己的线程ID，如果成功，则获取锁成功，如果失败，则说明有其他线程竞争锁，此时会转换为重量锁。

重量级锁：通过 Monitor 实现，本质是依赖底层操作系统的 MutexLock 实现。当多个线程竞争同一个锁时，会将锁对象放入一个同步队列中，等待获取锁的线程会进入同步队列中等待，当锁被释放时，会唤醒同步队列中的一个线程获取锁。
...
ReentrantLock是Java中的一个可重入锁，它是通过继承AQS（AbstractQueuedSynchronizer）实现的功能。

AQS的工作原理是基于一个双向链表的等待队列实现的，AQS中的同步状态是通过一个int类型的变量state来表示的。当一个线程获取锁时，会尝试使用CAS操作将state状态值修改为1，如果成功，则获取锁成功，如果失败，则说明有其他线程竞争锁，此时会进入等待队列中等待。

当锁的持有者释放锁时，会将state状态值减1，同时唤醒等待队列中的一个线程获取锁。如果等待队列中有多个线程在等待锁，则会按照先进先出的顺序唤醒等待队列中的线程。
